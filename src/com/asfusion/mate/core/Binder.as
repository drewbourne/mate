...
package com.asfusion.mate.core
{
	...
	public class Binder
	{
		...
		public function bind(scope:Object, target:Object, targetKey:String, source:Object, sourceKey:String):Boolean
		{
			...
			if (target && targetKey && source && sourceKey)
			{
				...
				try
				{
					if (soft)
					{
						var softWatcher:SoftChangeWatcher = bindProperty(target, targetKey, source, chainSourceKey);
						if (softWatcher.isWatching())
							isWatching = true;
					}
					else
					{
						var watcher:ChangeWatcher = BindingUtils.bindProperty(target, targetKey, source, chainSourceKey);
			...
			{
				if (!targetKey)
					logError(LogTypes.TARGET_KEY_UNDEFINED);
				else if (!target)
					logError(LogTypes.TARGET_UNDEFINED);
				else if (!source)
					logError(LogTypes.SOURCE_UNDEFINED);
			}
			
			if (!isWatching)
				logError(LogTypes.NOT_BINDING, null, null, null, { targetKey: targetKey });
		...
		private static var _keepFunctionLive:Array = new Array();
	}
}
