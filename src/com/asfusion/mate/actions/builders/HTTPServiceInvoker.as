	...
	public class HTTPServiceInvoker extends ServiceInvoker implements IAction
	{
		...
		*                                          Override protected methods
		...
		override protected function prepare(scope:IScope):void
		{
			super.prepare(scope);
			if (!currentInstance)
				currentInstance = new HTTPService();
			...
			if (channelSet)
				httpInstance.channelSet = channelSet;
			if (contentType)
				httpInstance.contentType = contentType;
			if (destination)
				httpInstance.destination = destination;
			if (headers)
				httpInstance.headers = headers;
			if (requestTimeoutChanged)
				httpInstance.requestTimeout = requestTimeout
			if (resultFormat)
				httpInstance.resultFormat = resultFormat;
			if (rootURL)
				httpInstance.rootURL = rootURL;
			if (proxyChanged)
				httpInstance.useProxy = useProxy;
			if (objectsBindableChanged)
				httpInstance.makeObjectsBindable = makeObjectsBindable;
